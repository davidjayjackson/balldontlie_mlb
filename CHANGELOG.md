...
### Changed
- All 11 functions now report `Category=MLB` (was the generic `Add-In`), so
  they're grouped together under their own category in Insert > Function
  Wizard instead of mixed in with every other installed add-in's functions.
